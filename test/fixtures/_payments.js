...
var state = {};
accounts.genesis = {
    address:  'gDTxidJjkmPw9TSrcz3mwrvdN5NpuKwMR1',
    secret: 's3TC7Hk4D5dVNyFtnZRz2e2nBnxYPQN2vw13Yo4xtdeWSQ2q369'
}
accounts.alice = {
    address : 'g941TxKXrhqgricDCpXpYBHdhK5StUWCPf',
    secret : 's3uHt9CEbqvA6sX23Cfxz6xJMcdceLcfHwACGU32NvLov1DKtET'
}
accounts.bob = {
    address : "ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY",
    secret : "s3kvbYKRAztPXw5bpHT39yrsmwZXNRTjaau4vKQRowrWh2ECGdd"
}
accounts.carol = {
    address : "g9WfuWxLqDEH2xLLcY85yK9qF31WsLRjYj",
    secret : "s3ioSqQsYjTb9fBKFekKyqi4W97MQWM2mUjefW5bmgWqQyJTk2g"
}
accounts.dan = {
    address : "gGpgSmraWsYN7NipMgbJEjdsUxNidSQbgm",
    secret : "s3VNQkeeK9YVCrohhUQWMBFgVmuwuoPRfCmXJhfkYg8dXvVG1GG"
}
    ...
    switch (data.source_account) {
        case accounts.alice.address :
            if (data.destination_account == accounts.bob.address) {
                if (data.destination_amount == '1') {
                    ws.send(JSON.stringify({
                        "id":data.id,"result":{"alternatives":[],"destination_account":"ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY","destination_currencies":["XRP"],"ledger_current_index":3,"validated":false},"status":"success","type":"response"
                    }));
                    ...
                    if (data.destination_amount.currency == 'USD') {
                        if (state.usd === undefined) {
                            ...
                            state.usd = 1;
                            ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[],"destination_account":"ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY","destination_currencies":["XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
                        } else {
                            // second time, return path
                            state.usd++;
                            ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[{"paths_canonical":[],"paths_computed":[],"source_amount":{"currency":"USD","issuer":"g941TxKXrhqgricDCpXpYBHdhK5StUWCPf","value":"10"}}],"destination_account":"ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY","destination_currencies":["USD","XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
                        }
                    }
                }
            }
        ...
        case accounts.genesis.address : 
            switch (data.destination_account) {
                ...
                break;
                case accounts.carol.address: 
                ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[],"destination_account":"g9WfuWxLqDEH2xLLcY85yK9qF31WsLRjYj","destination_currencies":["XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
                break;
                case accounts.dan.address:
                ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[],"destination_account":"gGpgSmraWsYN7NipMgbJEjdsUxNidSQbgm","destination_currencies":["XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
                break;
                default:
                break;
            }
        ...
        case accounts.bob.address :
            if (data.destination_account == accounts.alice.address) {
                ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[],"destination_account":"g941TxKXrhqgricDCpXpYBHdhK5StUWCPf","destination_currencies":["XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}
                ))
            }
        break;
        case accounts.carol.address :
            if (data.destination_account == accounts.dan.address) {
                ws.send(JSON.stringify({"id":data.id,"result":{"alternatives":[{"paths_canonical":[],"paths_computed":[],"source_amount":{"currency":"USD","issuer":"g9WfuWxLqDEH2xLLcY85yK9qF31WsLRjYj","value":"10"}}],"destination_account":"gGpgSmraWsYN7NipMgbJEjdsUxNidSQbgm","destination_currencies":["USD","XRP"],"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
            }
        default:
        break;    
    }
    ...
    switch (data.account) {
        ...
        case accounts.carol.address:
            ...
            if (state.carolaccountinfo == 1) {
                ws.send(JSON.stringify({"id":data.id,"result":{"account_data":{"Account":"g9WfuWxLqDEH2xLLcY85yK9qF31WsLRjYj","Balance":"400000000","Flags":0,"LedgerEntryType":"AccountRoot","OwnerCount":0,"PreviousTxnID":"BA11014094AB9563200249DB5CF3E506F6A4A07EFBC75FDD5C31CDE26C6F4B7E","PreviousTxnLgrSeq":4,"Sequence":1,"index":"F0F3E450A8E548CBC9A6F1E05D4B2CCBFE0F945D46B813C5E59DCB0414EE09A5"},"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
            } 
        break;
        case accounts.dan.address :
            ws.send(JSON.stringify({"id":data.id,"result":{"account_data":{"Account":"gGpgSmraWsYN7NipMgbJEjdsUxNidSQbgm","Balance":"600000000","Flags":0,"LedgerEntryType":"AccountRoot","OwnerCount":0,"PreviousTxnID":"6EF920AE9D528C8D72C5BAFCE527561FADEFB5241BF80266E14F39EB3D34E24C","PreviousTxnLgrSeq":4,"Sequence":1,"index":"34FF1FC6B0F1A856A4417DDA0CC13683BB2AF87B3BCA5CE2F10828E972029140"},"ledger_current_index":4,"validated":false},"status":"success","type":"response"}))
        break;
        default :
        break;
    }
    ...
    switch (data.account) {
        case accounts.alice.address :
            ws.send(JSON.stringify({
                "id":data.id,
                "result":{
                "account":"g941TxKXrhqgricDCpXpYBHdhK5StUWCPf","lines":[]
                },
                "status":"success",
                "type":"response"
            }))
        ...
        case accounts.bob.address :
            ws.send(JSON.stringify({"id":data.id,
            "result":{"account":"ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY","lines":[]},"status":"success","type":"response"
            }))
        break; 
        default:
        break;
...
var tx = function(data,ws) {
    ws.send(JSON.stringify(
    {"id":data.id,"result":{"Account":"g941TxKXrhqgricDCpXpYBHdhK5StUWCPf","Amount":"200000000","Destination":"ghmt4X48tYa5aGSwGcjyAJdfPhiNnyoxYY","Fee":"12","Flags":0,"LastLedgerSequence":10,"Sequence":2,"SigningPubKey":"022E3308DCB75B17BEF734CE342AC40FF7FDF55E3FEA3593EE8301A70C532BB5BB","TransactionType":"Payment","TxnSignature":"3044022030A2F33C088F07CC86A87B8E82EFC95E1CE1D525A16EF2DC059485EB54D946BB022009482C8F4F14BC4E2E7FF188C95DB47EA84533BF9E1114C28E7997B1B439C46A","hash":"8EA3CF4D854669007058EB45E9860611CC24FEB655895E418A5C8BC5EA901D01"},"status":"success","type":"response"}
    ))
}
exports.tx = tx;
