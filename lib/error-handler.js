...
function handleError(error, req, res, next) {
  ...
  if (err_obj.error === 'remoteError') {
    if (err_obj.message === 'Account not found.') {
      ...
      err_obj.message = 'stellar-lib reported an error. If the problem persists, please try restarting the server. Error: ' + JSON.stringify(err_obj.message);
    }
  }
